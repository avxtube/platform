...
export function HomeFeed({ categories: availableCategories, videos: initialVideos, shorts: initialShorts, playlists: initialPlaylists, locale }: HomeFeedResponse & { locale: string }) {
  ...
  const categories = React.useMemo(() => [{ value: "all", label: home("categories.all") }, ...availableCategories.map((category) => ({ value: category, label: categoryKeys[category] ? home(`categories.${categoryKeys[category]}`) : category }))], [availableCategories, home])
