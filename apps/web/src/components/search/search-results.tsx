      ...
      {result.videos.length ? (
        <section className="space-y-5 py-6">
          ...
          {result.videos.map((video) => (
            <SearchVideoCard key={video.id} video={video} locale={locale} />
          ))}
        </section>
      ) : null}
      {result.shorts.length ? (
...
function SearchVideoCard({ video, locale }: { video: Video; locale: Locale }) {
  ...
  const published = Intl.DateTimeFormat(locale, {
    day: "numeric",
    month: "short",
    year: "numeric",
  ...
  return (
    <article className="group grid gap-4 sm:grid-cols-[minmax(260px,360px)_1fr]">
      <Link
        href={`/watch/${video.id}`}
      ...
      <div className="min-w-0 py-1">
        ...
        {video.channel ? (
          ...
          >
            <span className="grid size-7 place-items-center rounded-full bg-foreground text-[9px] font-bold text-background">
              {video.channel.name
                .split(/\s+/)
                .map((word) => word[0])
                .join("")
