...
export function WatchComments({ videoId, initialComments, initialNextCursor, locale }: { videoId: string; initialComments: WatchComment[]; initialNextCursor: string | null; locale: string }) {
  ...
  const loadMoreRef = useInfiniteLoad({
    hasMore: commentsPage.hasMore,
    loading: commentsPage.loading,
    error: commentsPage.error,
    loadMore: commentsPage.loadMore,
  ...
  function addComment() { if (!draft.trim()) return; commentsPage.prepend({ id: `comment-${Date.now()}`, author: t("you"), initials: "ME", message: draft.trim(), likeCount: 0, publishedAt: new Date().toISOString() }); setDraft("") }
  function addReply(parentId: string) { if (!replyDraft.trim()) return; commentsPage.updateItems((items) => items.map((item) => item.id === parentId ? { ...item, replies: [...(item.replies ?? []), { id: `reply-${Date.now()}`, author: t("you"), initials: "ME", message: replyDraft.trim(), likeCount: 0, publishedAt: new Date().toISOString() }] } : item)); setReplyDraft(""); setReplyTo(null) }

  return <section className="mt-8">
    <div className="flex items-center gap-4"><h2 className="text-lg font-bold">{t("comments", { count: comments.length })}</h2><button type="button" onClick={() => setSort((value) => value === "top" ? "newest" : "top")} className="flex items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold hover:bg-muted"><ListFilter className="size-4" />{t(sort === "top" ? "sortTop" : "sortNewest")}</button></div>
    <div className="mt-5 flex gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-foreground text-xs font-bold text-background">ME</span><div className="flex-1"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={t("addComment")} rows={1} className="w-full resize-none border-b bg-transparent px-1 py-2 text-sm outline-none focus:border-primary" /><div className="mt-2 flex justify-end gap-2"><button type="button" onClick={() => setDraft("")} className="rounded-full px-4 py-2 text-sm hover:bg-muted">{t("cancel")}</button><button type="button" disabled={!draft.trim()} onClick={addComment} className="flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50"><Send className="size-4" />{t("comment")}</button></div></div></div>
      ...
      {replyTo === comment.id ? <div className="mt-2 flex gap-2"><input autoFocus value={replyDraft} onChange={(event) => setReplyDraft(event.target.value)} placeholder={t("replyTo", { name: comment.author })} className="min-w-0 flex-1 border-b bg-transparent px-2 text-sm outline-none" /><button type="button" onClick={() => addReply(comment.id)} className="rounded-full bg-primary px-3 py-1.5 text-xs text-primary-foreground">{t("reply")}</button></div> : null}
    ...
    {commentsPage.error ? <button type="button" onClick={() => void commentsPage.loadMore()} className="mt-7 rounded-full border px-5 py-2.5 text-sm font-semibold hover:bg-muted">{t("retry")}</button> : null}
