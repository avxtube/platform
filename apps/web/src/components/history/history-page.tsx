...
export function HistoryPage({
  ...
  const visible = entries.filter(
    (entry) =>
      ...
      (!normalizedQuery ||
        `${entry.content.title} ${entry.content.channel?.name ?? ""}`
          .toLowerCase()
  ...
  return (
    <div className="grid gap-12 pb-10 lg:grid-cols-[minmax(0,1fr)_280px] xl:gap-20">
      <main className="min-w-0">
        <header>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(["all", "video", "short", "podcast", "music"] as const).map(
              (item) => (
                <button
                  key={item}
                  type="button"
                  ...
                  className={`rounded-lg px-3.5 py-2 text-sm font-semibold ${filter === item ? "bg-foreground text-background" : "bg-muted hover:bg-muted/80"}`}
                >
                  {t(`filters.${item}`)}
                </button>
              )
        ...
        {!visible.length ? (
          <div className="mt-10 rounded-xl border border-dashed py-16 text-center text-sm text-muted-foreground">
            {t("empty")}
          </div>
        ) : (
          <HistorySections
      ...
      <aside className="space-y-7 lg:sticky lg:top-24 lg:self-start">
        <label className="flex items-center gap-3 border-b pb-3">
        ...
        <nav
          className="space-y-4 pl-8 text-sm text-muted-foreground"
          aria-label={t("otherHistory")}
        >
          <p>{t("comments")}</p>
          <p>{t("posts")}</p>
          <p>{t("liveChat")}</p>
          ...
          >
            <CarouselContent className="-ml-3">
              {shorts.map((entry) => (
                <CarouselItem
                  key={entry.id}
                  className="basis-1/2 pl-3 sm:basis-1/3 xl:basis-1/4"
                >
                  <ShortCard
                    entry={entry}
                    locale={locale}
            ...
            <CarouselPrevious
              className="-left-10"
              aria-label={t("previousShorts")}
            />
            <CarouselNext className="-right-10" aria-label={t("nextShorts")} />
          </Carousel>
        </section>
      ) : null}
...
function ShortCard({
  ...
  return (
    <article className="group min-w-0">
      <Link href={`/shorts/${entry.content.id}`} className="block">
        <div className="relative aspect-[9/14] overflow-hidden rounded-xl bg-muted">
          <img
