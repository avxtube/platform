...
const webLoaders = {
  en: async () => {
    const [common, auth, page, viewer, video, admin] = await Promise.all([
      ...
      import("./locales/en/admin.json").then((item) => item.default),
    ])
    return { common, auth, page, viewer, video, admin }
  },
  th: async () => {
    const [common, auth, page, viewer, video, admin] = await Promise.all([
      ...
      import("./locales/th/admin.json").then((item) => item.default),
    ])
    return { common, auth, page, viewer, video, admin }
  },
} satisfies Record<Locale, () => Promise<Messages>>
